...
export default function ChoosePlan() {
  ...
  useEffect(() => {
    ...
    const fetchEntity = async () => {
      ...
      if (!targetEntityId) {
        const { data: entityData } = await db.from('entities').select('id').eq('user_id', user.id).limit(1).maybeSingle();
    ...
    fetchEntity();
  }, [isAuthenticated, user, navigate, db, refreshUser]);
  ...
  const initMercadoPago = useCallback(async () => {
      ...
      const cardForm = mp.cardForm({
        ...
        callbacks: {
          onFormMounted: (error: any) => {
            if (error) { toast.error('Erro ao carregar formulário de pagamento'); return; }
            setMpReady(true);
          },
    ...
    } catch (error) { toast.error('Erro ao inicializar pagamento'); }
  }, []);

  useEffect(() => {
    if (showCardForm) {
      const timer = setTimeout(() => { initMercadoPago(); }, 100);
      return () => { clearTimeout(timer); if (cardFormRef.current) { try { cardFormRef.current.unmount(); } catch (_) {} } };
    }
  ...
  const handleCardSubmit = async (cardForm: any) => {
    ...
    try {
      ...
      if (!formData.token) { toast.error('Erro ao processar dados do cartão.'); setIsSubmitting(false); return; }
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL}/create-subscription`, {
        ...
        body: JSON.stringify({ entityId: currentEntityId, planName: 'pro', price: 39, cardTokenId: formData.token }),
      });

      if (!response.ok) { const errorData = await response.json(); throw new Error(errorData.error || 'Erro ao processar assinatura'); }
    ...
    } catch (error: any) { toast.error(error.message || 'Erro ao criar assinatura'); } finally { setIsSubmitting(false); }
  };
    ...
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-background via-background to-primary/5 p-4">
      <div className="w-full max-w-lg mx-4">
        <div className="text-center mb-8 space-y-4">
          ...
          <Button variant="ghost" className="text-muted-foreground" onClick={async () => { await supabase.auth.signOut(); navigate('/'); }}>← Sair da conta</Button>
        </div>
            ...
            {!showCardForm ? (
              <Button onClick={async () => {
                if (!entityId && !entityIdRef.current) {
                  ...
                  const { data } = await db.from('user_entity_access').select('entity_id').eq('user_id', user?.id).limit(1).maybeSingle();
                  targetEntityId = data?.entity_id;
                  if (!targetEntityId) { const { data: entData } = await db.from('entities').select('id').eq('user_id', user?.id).limit(1).maybeSingle(); targetEntityId = entData?.id; }
                  if (targetEntityId) { setEntityId(targetEntityId); entityIdRef.current = targetEntityId; setShowCardForm(true); } 
                  else toast.error('Nenhuma empresa encontrada. Recarregue (F5).');

# Content from https://raw.githubusercontent.com/asternic/wuzapi/main/API.md
...
2\. \*\*Admin Token\*\*: For admin endpoints (/admin/\*\*), use the \`Authorization\` header with the admin token value (set in WUZAPI\_ADMIN\_TOKEN).
...
The following admin-only endpoints are used to manage users in the system. All require the Authorization header with the admin token (WUZAPI\_ADMIN\_TOKEN).
...
Example Request:
\`\`\`
curl -s -X GET -H 'Authorization: {{WUZAPI\_ADMIN\_TOKEN}}' http://localhost:8080/admin/users
\`\`\`
...
Example Request:
\`\`\`
curl -s -X POST -H 'Authorization: {{WUZAPI\_ADMIN\_TOKEN}}' -H 'Content-Type: application/json' --data '{"name":"usuario2","token":"token2","webhook":"https://example.com/webhook2","events":"Message,ReadReceipt"}' http://localhost:8080/admin/users
\`\`\`
...
Example Request:
\`\`\`
curl -s -X DELETE -H 'Authorization: {{WUZAPI\_ADMIN\_TOKEN}}' http://localhost:8080/admin/users/2
\`\`\`
...
The following \_webhook\_ endpoints are used to get or set the webhook that will be called whenever a message or event is received. Available event types are:
...
The following \_HMAC\_ endpoints are used to configure and manage HMAC keys for webhook security. HMAC signatures verify that webhooks are authentic and haven't been tampered with.
...
Endpoint: \_/session/hmac/config\_
...
\`\`\`json
{
 "hmac\_key": "your\_hmac\_key\_minimum\_32\_characters\_long\_here"
}
\`\`\`
...
\`\`\`
curl -s -X POST -H 'Authorization: 1234ABCD' -H 'Content-Type: application/json' --data '{"hmac\_key":"your\_hmac\_key\_minimum\_32\_characters\_long\_here"}' http://localhost:8080/session/hmac/config
\`\`\`
...
The following \_session\_ endpoints are used to start a session to Whatsapp servers in order to send and receive messages
...
Endpoint: \_/session/disconnect\_
...
The following \_user\_ endpoints are used to gather information about Whatsapp users.

\## Gets user details

Gets information for users on Whatsapp

Endpoint: \_/user/info\_
...
\`\`\`json
{
 "URL": "https://pps.whatsapp.net/v/t61.24694-24/227295214\_112447507729487\_4643695328050510566\_n.jpg?stp=dst-jpg\_s96x96&ccb=11-4&oh=ja432434a91e8f41d86d341bx889c217&oe=543222A4",
 "ID": "1645308319",
 "Type": "preview",
 "DirectPath": "/v/t61.24694-24/227295214\_112447507729487\_4643695328050510566\_n.jpg?stp=dst-jpg\_s96x96&ccb=11-4&oh=ja432434a91e8f41d86d341ba889c217&oe=543222A4"
}
\`\`\`
...
The following \_chat\_ endpoints are used to send messages or mark them as read or indicating composing/not composing presence. The sample response is listed only once, as it is the
...
Endpoint: \_/chat/send/template\_
...
Endpoint: \_/chat/send/document\_
...
Endpoint: \_/chat/send/sticker\_
...
Endpoint: \_/chat/send/location\_
...
Endpoint: \_/chat/send/contact\_
...
endpoint: \_/chat/downloadimage\_
...
endpoint: \_/chat/downloadvideo\_
...
endpoint: \_/chat/downloadaudio\_
...
endpoint: \_/chat/downloaddocument\_
...
The following \_group\_ endpoints are used to gather information or perfrom actions in chat groups.

\## List subscribed groups

Returns complete list of subscribed groups

endpoint: \_/group/list\_
...
endpoint: \_/group/photo/remove\_
...
\`\`\`json
{
 ...
 "path\_style": false,
 "public\_url": "https://cdn.example.com",
 "media\_delivery": "both",
 "retention\_days": 30
}
\`\`\`
...
\- \`access\_key\`: S3 access key ID
\- \`secret\_key\`: S3 secret access key
\- \`path\_style\`: Use path-style URLs (required for MinIO)
\- \`public\_url\`: Custom public URL for accessing files (optional)
\- \`media\_delivery\`: Delivery method - "base64", "s3", or "both"
\- \`retention\_days\`: Days to retain files (0 for no expiration)
...
\`\`\`json
{
 ...
 "region": "us-west-004",
 "bucket": "my-b2-bucket",
 "access\_key": "0004b0000000000000000000001",
 "secret\_key": "K004XXXXXXXXXXXXXXXXXXXXXXXX",
 "path\_style": false,
 "media\_delivery": "s3",
 "retention\_days": 90
}
...
\`\`\`
users/{user\_id}/{inbox\|outbox}/{contact\_jid}/{year}/{month}/{day}/{media\_type}/{message\_id}.{ext}
\`\`\`

Example:
\`\`\`
users/abc123/inbox/5491155553934\_s.whatsapp.net/2024/12/25/images/3EB06F9067F80BAB89FF.jpg
\`\`\`

\## Webhook Payload

When S3 is enabled, webhook payloads will include S3 information based on the \`media\_delivery\` setting:

\### S3 Only (\`media\_delivery: "s3"\`)
\`\`\`json
{
 "event": {
 "Info": { ... },
...
\### Both S3 and Base64 (\`media\_delivery: "both"\`)
\`\`\`json
{
 "event": { ... },
...
4\. \*\*Fallback\*\*: If S3 upload fails, the webhook is still sent (without S3 data if \`media\_delivery\` is "s3" only).
...
1\. Configure S3 with \`media\_delivery: "both"\` initially
2\. Update your webhook handler to support both formats
3\. Test thoroughly with various media types
4\. Switch to \`media\_delivery: "s3"\` once confirmed working
...
\- For MinIO, ensure \`path\_style: true\` is set

\### Files Not Accessible
\- Check bucket policy allows public read
\- Verify CORS settings if accessing from browser
\- Ensure \`public\_url\` is correct if using CDN

\### Webhook Missing S3 Data
\- Check \`media\_delivery\` setting
...
Starting from version X.X.X, you can choose the format for sending webhook data using the \`WEBHOOK\_FORMAT\` environment variable.
...
\`\`\`bash
export WEBHOOK\_FORMAT=json # or "form" for the default
\`\`\`
...
jsonData={...json...}&token=YOUR\_TOKEN
\`\`\`
